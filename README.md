...
- **四级数据降级**：华尔街见闻 → akshare → 东方财富 → 静态兜底，任意数据源故障自动切换
...
StockMonitor/
│
├── server.py               # Flask 后端（主入口）
├── dashboard.html          # 前端仪表盘（单文件，无需构建）
│
├── config.py               # YAML 配置读取
├── config.yaml             # 配置文件模板（填入 API Key 后使用）
│
├── fetcher.py              # A 股 K 线数据（akshare）
├── indicators.py           # 技术指标：MACD / 布林带 / RSI / KDJ / VWAP / OBV / 均线
├── analysis.py             # 支撑压力位识别（三法融合）
├── ai_advisor.py           # 股票 AI 分析（DeepSeek，支持思考模式）
├── decision.py             # 规则引擎 + AI 综合决策
│
├── sector_fetcher.py       # 板块联动：行业识别 + 成分股涨跌
├── capital_fetcher.py      # 北向资金 + 融资融券
├── calendar_fetcher.py     # 财报 / 分红日历
│
├── commodity_fetcher.py    # 大宗商品行情（华尔街见闻 / akshare）
├── commodity_ai.py         # 大宗商品 AI 产业影响分析
│
├── scheduler.py            # 定时调度
├── logger_setup.py         # 日志（loguru）
├── ws_test.py              # WebSocket 推送可行性验证（独立脚本）
│
